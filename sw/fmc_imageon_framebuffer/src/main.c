...
void ambient_light_loop(fmc_imageon_hdmi_framebuffer_t *config, Pwm* pwm)
{
	...
	while(1)
	{
		...
		//led top left
		leds[6] = get_led_avg(video_data, 0, HORIZONTAL_WIDTH_SMALL, 0, VERTICAL_WIDTH_SMALL);

		//led top mid left
		leds[5] = get_led_avg(video_data, HORIZONTAL_WIDTH_SMALL, HORIZONTAL_WIDTH_LARGE, 0, ambient_depth);

		//led top mid right
		leds[4] = get_led_avg(video_data, HORIZONTAL_WIDTH_SMALL + HORIZONTAL_WIDTH_LARGE, HORIZONTAL_WIDTH_LARGE, 0, ambient_depth);
		...
		//led left
		leds[7] = get_led_avg(video_data, 0, (ambient_depth / 2), VERTICAL_WIDTH_SMALL, VERTICAL_WIDTH_LARGE);

		//led right
		leds[2] = get_led_avg(video_data, COLS - (ambient_depth / 2), (ambient_depth / 2), VERTICAL_WIDTH_SMALL, VERTICAL_WIDTH_LARGE);

		//led bottom left
		leds[8] = get_led_avg(video_data, 0, HORIZONTAL_WIDTH_SMALL, ROWS - VERTICAL_WIDTH_SMALL, VERTICAL_WIDTH_SMALL);

		//led bottom mid left
		leds[9] = get_led_avg(video_data, HORIZONTAL_WIDTH_SMALL, HORIZONTAL_WIDTH_LARGE, ROWS - ambient_depth, ambient_depth);

		//led bottom mid right
		leds[0] = get_led_avg(video_data, HORIZONTAL_WIDTH_SMALL + HORIZONTAL_WIDTH_LARGE, HORIZONTAL_WIDTH_LARGE, ROWS - ambient_depth, ambient_depth);

		//led bottom right
		leds[1] = get_led_avg(video_data, HORIZONTAL_WIDTH_SMALL + 2 * HORIZONTAL_WIDTH_LARGE, HORIZONTAL_WIDTH_SMALL, ROWS - VERTICAL_WIDTH_SMALL, VERTICAL_WIDTH_SMALL);
		...
		else
		{
			parkptr &= ~0x1;
			XAxiVdma_WriteReg(config->vdma_hdmi.BaseAddr, XAXIVDMA_PARKPTR_OFFSET, parkptr);
			pMM2S_Mem[0].value = video_data[0].value;
		}
	}
		...
		int ambient_depth)
{
	...
	for(i = 0; i < NUM_PIXELS; i++)
	{
		...
		else if((x == HORIZONTAL_WIDTH_SMALL)
				&& ((y < LARGER(ambient_depth, VERTICAL_WIDTH_SMALL))
				|| y > HEIGHT - LARGER(ambient_depth, VERTICAL_WIDTH_SMALL)))
		{

			video_data_out[i].value = WHITE;
		}
		else if(x == HORIZONTAL_WIDTH_SMALL + HORIZONTAL_WIDTH_LARGE
				&& (y < ambient_depth || y > HEIGHT - ambient_depth))
		{
			video_data_out[i].value = WHITE;
		}
		else if(x == HORIZONTAL_WIDTH_SMALL + 2 * HORIZONTAL_WIDTH_LARGE
				&& (y < LARGER(ambient_depth, VERTICAL_WIDTH_SMALL)
				|| y > HEIGHT - LARGER(ambient_depth, VERTICAL_WIDTH_SMALL)))
		{
			video_data_out[i].value = WHITE;
		...
		else if(y == VERTICAL_WIDTH_SMALL
				&& (x < LARGER(HORIZONTAL_WIDTH_SMALL, (ambient_depth / 2))
				|| x > WIDTH - LARGER(HORIZONTAL_WIDTH_SMALL, (ambient_depth / 2))))
		{
			video_data_out[i].value = WHITE;
		}
		else if(y == HEIGHT - VERTICAL_WIDTH_SMALL
				&& (x < LARGER(HORIZONTAL_WIDTH_SMALL, (ambient_depth / 2))
				|| x > WIDTH - LARGER(HORIZONTAL_WIDTH_SMALL, (ambient_depth / 2))))
		{
			video_data_out[i].value = WHITE;
		...
		//ambient depth
				...
				&& x < WIDTH - HORIZONTAL_WIDTH_SMALL
				&& y == ambient_depth)
		{
			video_data_out[i].value = WHITE;
		}
				...
				&& x < WIDTH - HORIZONTAL_WIDTH_SMALL
				&& y == HEIGHT - ambient_depth)
		{
			video_data_out[i].value = WHITE;
		}
		else if(x == (ambient_depth / 2)
				&& y > VERTICAL_WIDTH_SMALL
				&& y < HEIGHT - VERTICAL_WIDTH_SMALL)
		{
			video_data_out[i].value = WHITE;
		}
		else if(x == WIDTH - (ambient_depth / 2)
				&& y > VERTICAL_WIDTH_SMALL
				&& y < HEIGHT - VERTICAL_WIDTH_SMALL)
		{
			video_data_out[i].value = WHITE;
